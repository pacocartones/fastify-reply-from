...
function buildRequest (opts) {
  ...
  if (isHttp2) {
    ...
    if (isBalanced) {
      ...
      /* c8 ignore start -- unix sockets are not supported/testable on Windows */
  ...
  function handleUndici (opts, done) {
    ...
    if (undiciInstance) {
      ...
      if (pool instanceof undici.BalancedPool) {
        delete req.origin
      }
    /* c8 ignore start -- unix sockets are not supported/testable on Windows */
  ...
  function handleHttp2Req (opts, done) {
    ...
    if (!http2Client || http2Client.destroyed || http2Client.closed) {
      /* c8 ignore next -- defensive cleanup for a closed but not yet destroyed HTTP/2 session */
...
function end (req, body, cb) {
  ...
  /* c8 ignore start -- unreachable with supported body types */
  } else {
